...
\large A First Closure-Projection Channel in $\Vsix$}
\author{%
  Lee Smart\\[4pt]
...
first local-to-global spectral channel in the $\Vsix$ programme.
...
\begin{itemize}
  \item \url{https://github.com/vfd-org/the-24-600-spectral-bridge}
        (hosts both Paper 1 and Paper 2 alongside this synthesis;
        run \texttt{pytest closure\_transform\_engine/tests/} to
        re-assert every certificate-level claim of both prior
        notes).
\end{itemize}
